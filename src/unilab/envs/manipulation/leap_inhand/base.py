"""LEAP Hand 共用的 16-DoF 環境基礎層。

這個檔案只處理動作空間、關節控制和球/指尖狀態讀取，不依賴 Allegro。
"""
...
class LeapHandBaseEnv(NpEnv):
    """LEAP 手的 backend-independent 狀態與 position-target 控制介面。"""
    _NUM_HAND_DOF: int = 16
    _FINGERTIP_BODY_NAMES: tuple[str, ...] = ("ff_tip", "mf_tip", "rf_tip", "th_tip")
    _cfg: LeapHandBaseCfg
    _init_qpos: np.ndarray
    _init_qvel: np.ndarray

    def __init__(self, cfg: LeapHandBaseCfg, backend: SimBackend, num_envs: int = 1):
    ...
    def _resolve_init_qpos(self) -> np.ndarray:
        """只在初始化時從 task-level keyframe 取得完整 qpos。"""
    ...
    def apply_action(self, actions: np.ndarray, state: NpEnvState) -> np.ndarray:
        """把 [-1, 1] policy action 積分成下一個關節 position target。"""
...
LeapHandBaseMjEnv = LeapHandBaseEnv
